return function(Prefs)
	for pn = 1,2 do
		if FILEMAN:DoesFileExist("Save/MachineProfile/OutFoxPrefsForPlayerp"..pn.."/Infinitesimal.ini") then 
			local Createfile = RageFileUtil.CreateRageFile()
			Createfile:Open("Save/MachineProfile/OutFoxPrefsForPlayerp"..pn.."/Infinitesimal.ini", 2)
			Createfile:Write("")
			Createfile:Close()
			Createfile:destroy()
		end
